...
ns3::Ptr<ns3::Node> LeachNodeApplication::GetTargetNode(){
    return m_targetNode;
}

ns3::Time LeachNodeApplication::GetMTBEvents(){
    return m_mtbEvents;
}

void LeachNodeApplication::SetTargetNode(ns3::Ptr<ns3::Node> target){
    m_targetNode = target;
}

void LeachNodeApplication::SetMTBEvents(ns3::Time time){
    m_mtbEvents = time;
}
...
/*class LeachNodeAplication: protected ns3::Application{
    public:
        LeachNodeAplication();

        ~LeachNodeAplication() override;

        void SetClusterHead(ns3::Node node);

        void SetBaseStation(ns3::Node node);

        void SetCHProb(float probability);

    private:
        void StartApplication() override;
        void StopApplication() override;
        void ScheduleTransimt(ns3::Time dt);
        void Send();
        void RelayPacket(ns3::Socket socket);

        ns3::Node m_clusterHead;
        ns3::Node m_baseStation;
...
int main(int argc, char* argv[]){
    ...
    // Run Sim
    ns3::Simulator::Schedule(ns3::MilliSeconds(5), &test);
